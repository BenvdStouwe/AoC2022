...
R 2";
    ...
    private const string SecondTestInput = @"R 5
U 8
L 8
D 3
R 17
...
U 20";
    ...
    [Theory]
    [InlineData(TestInput, 1)]
    [InlineData(SecondTestInput, 36)]
    ...
    private static int Solve(string input, int ropeLength)
    {
        ...
        foreach (var (direction, amount) in ParseInput(input))
        {
            for (var i = 1; i <= amount; i++)
            {
                locations[0] = CalculateNewLocation(direction, locations[0]);
                for (var j = 1; j < locations.Length; j++)
                {
                    var xDiff = Math.Abs(locations[j - 1].x - locations[j].x);
                    var yDiff = Math.Abs(locations[j - 1].y - locations[j].y);
                    if (xDiff + yDiff > 2)
                    {
    ...
    private static IEnumerable<(char direction, short amount)> ParseInput(string input) =>
        input.Split(Environment.NewLine)
            .Select(l => (direction: l[0], amount: short.Parse(l[2..])));

    private static (int x, int y) ConsolidateLocations((int x, int y) leadLocation, (int x, int y) followLocation,
        int xDiff, int yDiff) =>
        yDiff == 2 && xDiff == 2
            ? (Math.Max(leadLocation.x, followLocation.x) - 1, Math.Max(leadLocation.y, followLocation.y) - 1)
            : (yDiff == 2 ? leadLocation.x : Math.Max(leadLocation.x, followLocation.x) - 1,
                xDiff == 2 ? leadLocation.y : Math.Max(leadLocation.y, followLocation.y) - 1);

    private static (int x, int y) CalculateNewLocation(char direction, (int x, int y) location) =>
        direction switch
        {
            'U' => (location.x, location.y + 1),
            'D' => (location.x, location.y - 1),
            'L' => (location.x - 1, location.y),
            _ => (location.x + 1, location.y)
        };

    private const string RealInput = @"R 2
D 2
U 2
D 1
L 1
